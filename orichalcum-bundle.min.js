...
o.version = '1.0.0';
o.plugins = {
href:function(e, v){e.href = v;if(v.indexOf('://') == -1){e.addEventListener('click',(event)=>{event.preventDefault();o.route(event.target.closest('a').getAttribute('href'))})}return e},fetch:function(element, values){if(!values.options){values.options = {method: 'GET'}}if(!values.options.credentials){values.options.credentials = 'include'}if(!values.type){values.type = 'json'}values.element = element;values.next = [];o.fetch(values);return element},
fetch:function(element, values){if(!values.options){values.options = {method: 'GET'}}if(!values.options.credentials){values.options.credentials = 'include'}if(!values.type){values.type = 'json'}values.element = element;values.next = [];o.fetch(values);return element},
form:function(e, v){e.addEventListener('submit',(event)=>{event.preventDefault();let data = {};let form = new FormData(event.target);for(const [key, value] of form){data[key] = value}v(event, e, data)});return e},
}	
